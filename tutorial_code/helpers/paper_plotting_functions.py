...
def paper_plots_example1_1(experiment_data_list, indices_to_plot="first"):
    ...
    fig, axs = plt.subplots(
        3, 3, figsize=(6 * num_experiments, 15)
    )  # Adjust figsize for width and height
    fig.canvas.manager.set_window_title(
        "Figure 6: Balancing qubits in a FM chain with flux-bias offsets"
    )
...
def paper_plots_example1_2(
    *, all_couplings, all_fbos, mags, frust, indices_to_plot="first"
    ...
    fig.canvas.manager.set_window_title(
        "Figure 7: Balancing qubits and couplers in a FM chain with flux-bias offsets and coupler adjustments"
    )
    ...
    if MAKE_TIKZ_PLOTS:
        ...
        for i, ax in enumerate(
            axs.flatten()
        ):  # Flatten the 2x2 grid to iterate over all subplots
            fn = file_names[i]
            code = tikzplotlib.get_tikz_code(
                figure=fig,
                axis=ax,
...
def paper_plots_example2_2(
    *, nominal_couplings, all_fbos, all_couplings, mags, frust, indices_to_plot="first"
    ...
    if MAKE_TIKZ_PLOTS:
        ...
        for i, ax in enumerate(
            axs.flatten()
        ):  # Flatten the 2x2 array of axes for iteration
            fn = file_names[i]
            code = tikzplotlib.get_tikz_code(
...
def paper_plots_example3_2(
    *,
    ...
    if type_ == "embedded_finite":
        ...
        fig.canvas.manager.set_window_title(
            "Figure 15: Shimming an isotropic, infinite triangular antiferromagnet (halved boundary couplers)"
        )
...
def paper_plots_example3_2_heatmaps(experiment_data_list):
    ...
    for row, data in enumerate(experiment_data_list):
        ...
        axs[row, 0].set_aspect(
            1.0 / axs[row, 0].get_data_ratio(), adjustable="box"
        )  # Square aspect ratio
        ...
        for col, psi in enumerate(psi_data):
            ...
            # Add reference lines
            ax.plot(
                [-1 / np.sqrt(3), 1 / np.sqrt(3)], [-1, 1], color="w", linestyle="-"
            )
            ax.plot(
                [-1 / np.sqrt(3), 1 / np.sqrt(3)], [1, -1], color="w", linestyle="-"
            )
